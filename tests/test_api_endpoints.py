...
class TestAnalyzeEndpoint:
    ...
    def test_valid_analyze(self, client):
        response = client.post("/api/analyze", {"texts": ["This is amazing!", "This is terrible."]}, format="json")
        assert response.status_code == 200
        assert response.data["success"] is True
        assert "analysis" in response.data

    def test_analyze_single_text(self, client):
        response = client.post("/api/analyze", {"texts": ["I love this product!"]}, format="json")
        assert response.status_code == 200
